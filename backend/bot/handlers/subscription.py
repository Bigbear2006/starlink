...
async def display_subscription_plans(msg: Message, state: FSMContext):
    try:
        ...
        subscription_end: datetime = earliest_payment.date + timedelta(days=30 * payments_count)
        remaining_days = (subscription_end - datetime.now(timezone(conf.settings.TIME_ZONE))).days
        ...
        await msg.answer(
            f'Ваш тариф: {SubscriptionPlanChoices(earliest_payment.subscription_plan).label}\n'
            f'Дата последнего продления подписки: {datetime.astimezone(pytz.utc).strftime(last_payment.date, settings.DATE_FMT)}\n'
            f'Ваша подписка закончится {datetime.strftime(subscription_end, settings.DATE_FMT)} '
...
async def check_subscription_prolonging(query: CallbackQuery, state: FSMContext):
    check_payment = True
    if check_payment:
        await Payment.objects.acreate(
            client_id=query.message.chat.id,
